...
void Reassembler::insert(uint64_t first_index, string data,
    ...
    if(data.empty()) return;
      ...
      * max_idx 不可以写入数据的最小下标，接收到最后一条子串时更新，如果当前子串超出了max_idx，截断子串并更新max_idx
      * nxt_idx 如果first_index是nxt_idx,那么就可以直接插入，每次写入Writer stream的时候更新
      *         而且更新nxt_idx时都需要检验之前暂存起来的子串是否可以接入Writer stream了
      * max_idx和nxt_idx都是尚未被数据填充的
      */
     if(first_index>=max_idx || first_index+data.size()<nxt_idx) return;
    ...
    if (end_checked && nxt_idx == end_idx) {
        output.close();
    }
}
...
void Reassembler::store(uint64_t idx,std::string str){
    ...
    for (auto iter = buf.begin(); iter != buf.end();) {
        ...
        if ((idx >= iter->first && idx <= ite_end) || (iter->first >= idx && iter->first <= str_end)) {
            merge(idx,str, iter->first,iter->second);  // 先消除重叠：把传入的 seg 与已缓存并存在重叠的 segment 合并,
            unassembled_bytes-=iter->second.size();
...
void Reassembler::merge(uint64_t idx,string _str,uint64_t bdx, string _buf) {
